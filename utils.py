...
def get_optimizer(net, optimizer_name, scheduler_name, optimizer_settings, scheduler_settings):
    if optimizer_name == 'Adam':
        ...
        #UIUNet optimizer = torch.optim.Adam(net.parameters(), lr=0.001, betas=(0.9, 0.999), eps=1e-08, weight_decay=0)
    elif optimizer_name == 'Adagrad':
        #optimizer  = torch.optim.Adagrad(net.parameters(), lr=optimizer_settings['lr'])
        optimizer = torch.optim.Adagrad(net.parameters(), lr=optimizer_settings['lr'], weight_decay=1e-4)
...
def padimg_patch(img,patch_size):
    h, w = img.shape
    if min(h, w) < patch_size:
        img = np.pad(img, ((0, max(h, patch_size)-h),(0, max(w, patch_size)-w)), mode='constant')
    return img

def crop_img(images, label, patch_size):
    h, w = images.shape
    if h>patch_size or w>patch_size:
        # images = torch.from_numpy(images)
        # label = torch.from_numpy(label)

        labelimage = measure.label(label, connectivity=2)
        props = measure.regionprops(labelimage, intensity_image=label, cache=True)
        prob = random.uniform(0, 1)
        ps = patch_size
        if len(props) > 0 and prob < 0.75:
            idx = torch.randint(0, len(props), (1, 1))
            r, c = props[idx].centroid
            if r > h - ps / 2:
                r = h - ps / 2
            if c > w - ps / 2:
                c = w - ps / 2
            ...
            r = torch.randint(20, h - 128, (1, 1))
            c = torch.randint(20, h - 128, (1, 1))
        r0 = max(0, r - int(ps / 2))
        c0 = max(0, c - int(ps / 2))

        images = images[r0:r0 + ps, c0:c0 + ps]
        label = label[r0:r0 + ps, c0:c0 + ps]
    return images, label