...
parser.add_argument("--model_names", default=['FM-Net'], nargs='+',
                    help="model_name: 'ACM', 'ALCNet', 'DNANet', 'ISNet', 'UIUNet', 'RDIAN', 'ISTDU-Net', 'U-Net', 'RISTDnet','SCTransNet_WTConv_MaskAtt_FSNet'")
...
parser.add_argument("--Train_batchSize", type=int, default=8, help="Training batch sizse. UIU 3 DNANet 16")
parser.add_argument("--Test_batchSize", type=int, default=1, help="Training batch sizse. UIU 3 DNANet 16")
parser.add_argument("--patchSize", type=int, default=512, help="Training patch size UIU 320 DNANet 256")
...
parser.add_argument("--nEpochs", type=int, default=1500, help="Number of epochs UIU 500 DNANet 1500")
parser.add_argument("--begin_test", default=1, type=int, help='Number of epochs to begin testing')
parser.add_argument("--step_test", default=1, type=int, help='Step size of testing')
...
def train():
    ...
    train_loader = DataLoader(dataset=train_set, num_workers=opt.threads, batch_size=opt.Train_batchSize, shuffle=True)
    
    net = Net(model_name=opt.model_name, mode='train', patch_size=opt.patchSize).cuda()
    net.train()
    ...
    for idx_epoch in range(epoch_state, opt.nEpochs):
        for idx_iter, (img, gt_mask) in enumerate(tqdm(train_loader),0):
        ...
        if (idx_epoch + 1) % opt.step_test == 0 and (idx_epoch+1) >= opt.begin_test:
            ...
            results1, results2 = test(save_pth)
            if results1[1] > best_mIOU:
                best_mIOU = results1[1]
...
def test(save_pth):
    test_set = TestSetLoader(opt.dataset_dir, opt.dataset_name, opt.dataset_name, img_norm_cfg=opt.img_norm_cfg, patchsize=opt.patchSize)
    test_loader = DataLoader(dataset=test_set, num_workers=1, batch_size=opt.Test_batchSize, shuffle=False)
    
    net = Net(model_name=opt.model_name, mode='test', patch_size=opt.patchSize).cuda()
    ckpt = torch.load(save_pth)
    net.load_state_dict(ckpt['state_dict'])
    net.eval()

    eval_mIoU = mIoU()
    eval_PD_FA = PD_FA_train()

    with torch.no_grad():
        for idx_iter, (img, gt_mask, size, _) in enumerate(tqdm(test_loader),0):
