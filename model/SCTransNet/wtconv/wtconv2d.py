...
class WTConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, bias=True, wt_levels=1, wt_type='db1'):
        ...
        self.ca = nn.ModuleList([ChannelAttention(in_channels*4) for _ in range(self.wt_levels)])
        self.sa = nn.ModuleList([SpatialAttention() for _ in range(self.wt_levels)])
    ...
    def forward(self, x):
        ...
        for i in range(self.wt_levels):
            ...
            curr_x_tag = self.sa[i](self.ca[i](self.wavelet_convs[i](curr_x_tag)))
