...
class UNet_F(nn.Module):
    def __init__(self, n_classes=1, layer=4, mode=1, n_channels=3,se=False,bilinear=True,patchSize=128,interpolate_mode='bicubic'):
        super(UNet_F, self).__init__()
        self.n_classes = n_classes
        self.in_resizer = Resizer(n_channels=n_channels, patchsize=patchSize,scale=3)
       # self.saa = SAA(cfg)
        self.layer = layer
        self.mode = mode
        ...
        if self.mode == 1:
            if self.layer == 4:
                ...
                self.down4 = (last_dwt1(512, 1024, self.se))

                self.up4 = (part_iwt1_fuse(128, 512, self.bilinear))
                ...
                self.up3 = (part_iwt1(512, 256, self.bilinear))
                # self.conv3 = nn.Conv2d(256, self.n_classes, 1)
                self.conv3 = (out_conv(256, 128, self.n_classes))
                
                self.up2 = (part_iwt1(256, 128, self.bilinear))
                # self.conv2 = nn.Conv2d(128, self.n_classes, 1)
                self.conv2 = (out_conv(128, 64, self.n_classes))

                self.up1 = (part_iwt1(128, 64, self.bilinear))
                self.conv1 = nn.Conv2d(64, self.n_classes, 1)

                self.out = (OutConv(self.n_classes * self.layer, self.n_classes, self.patchzise, self.interpolate_mode))
