...
parser.add_argument("--pth_dirs", default=['IRSTD-1K/FM-Net_IRSTD-1K.pth.tar'], nargs='+',  help="checkpoint dir, default=None or ['NUDT-SIRST/ACM_400.pth.tar','NUAA-SIRST/ACM_400.pth.tar']")
...
parser.add_argument("--patchSize", type=int, default=512, help="Testing patch size")
...
def test(): 
    test_set = TestSetLoader(opt.dataset_dir, opt.train_dataset_name, opt.test_dataset_name, opt.img_norm_cfg, patchsize=opt.patchSize)
    ...
    eval_mIoU = mIoU() 
    eval_PD_FA = PD_FA_test(200)
    with torch.no_grad():
        for idx_iter, (img, gt_mask, size, img_dir) in enumerate(tqdm(test_loader),0):
            img = Variable(img).cuda()
            pred = net.forward(img)
            pred = pred[:,:,:size[0],:size[1]]
            gt_mask = gt_mask[:,:,:size[0],:size[1]]
            eval_mIoU.update((pred>opt.threshold).cpu(), gt_mask)
            eval_PD_FA.update((pred[0,0,:,:]).cpu(), gt_mask[0,0,:,:], size)
    ...
    scio.savemat('test_ROC' + '/' + 'PD_FA_'+ (time.ctime()).replace(' ', '_').replace(':', '_') + '.mat',
             {'number_record1': results2[1], 'number_record2': results2[0]})
